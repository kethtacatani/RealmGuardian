...
func _process(delta):
	if cooldown_time!=global.dash_cooldown:
		cooldown_time= global.dash_cooldown
		timer.wait_time =cooldown_time
		label.text="%0.1f" % cooldown_time+"s"
	
	if not global.can_dash:
		label.text= "%0.1f" % timer.time_left
		progress_bar.value= int ((timer.time_left/cooldown_time)*100)
	
	if Input.is_action_just_pressed("dash") and not global.attacking and global.can_dash:
		if not isRun:
			isRun=true
			timer.start()
			label.show()
